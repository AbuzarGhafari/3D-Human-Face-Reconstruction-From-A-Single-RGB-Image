function [projectedPoints] = projection3D(vertices, direction, angle, p, ex, ey)
    ...
    H_matrix = [p, 0, ex; 0, p, ey; 0, 0, 1];   % 
        
    P_matrix = [1, 0, 0; 0, 1, 0];              % Ignore Z 
    
    M = R;
    
    projectedPoints = P_matrix * (M * vertices');
    
    projectedPoints = projectedPoints';
    
%     projectedPoints = 
    
%     intrinsic_matrix = [1, 1, 1; 1, 1, 1];
...
% 
%     % Apply projection
%     projectedPoints =  ( intrinsic_matrix .* projectionMatrix * rotatedVertices')';
end